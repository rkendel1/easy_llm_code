...
const [directoryArg, version, downloadURL] = process.argv.slice(2);
if (!directoryArg || !version || !downloadURL) throw new Error("Usage: node generate-release-metadata.mjs <artifact-directory> <version> <download-url>");
...
const values = { VERSION: version.replace(/^v/, ""), DOWNLOAD_URL: downloadURL.replace(/\/$/, ""), PUBLISHED_AT: new Date().toISOString() };
