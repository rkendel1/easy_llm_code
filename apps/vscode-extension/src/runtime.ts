...
export const parseRuntimeConnection = (output: string): { url: string; root: string; token: string } | undefined => { const url = output.match(/^URL:\s*(.+)$/m)?.[1]?.trim(), root = output.match(/^Root:\s*(.+)$/m)?.[1]?.trim(), token = output.match(/^Token:\s*(.+)$/m)?.[1]?.trim(); return url && root && token ? { url, root, token } : undefined; };
...
export class ExtensionRuntime {
  ...
  async indexProject(): Promise<{ files: number; commits: number }> { if (!this.client) throw new Error("Runtime is not connected"); const result = await this.client.indexProject(); await this.refresh(); return result; }
