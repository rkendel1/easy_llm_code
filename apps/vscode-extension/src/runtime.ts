...
export class ExtensionRuntime {
  ...
  async start(workspaceRoot: string, vaultPassword?: string): Promise<void> {
    ...
    const client = new RuntimeClient({ ...connection, pollIntervalMs: 200 }); this.client = client; await client.connect({ identity: { id: "vscode", name: "Visual Studio Code", adapterVersion: "0.2.0" }, capabilities }); this.unsubscribe = client.subscribeToEvents((value) => { this.event(value); void this.refresh(); }); await this.refresh();
  }
