...
export interface RuntimeSnapshot { status?: WorkspaceRuntimeStatus; tasks: RuntimeTask[]; connected: boolean; error?: string }
...
export class ExtensionRuntime {
  private process?: ChildProcessWithoutNullStreams; private client?: RuntimeClient; private unsubscribe?: () => void; private root = ""; private snapshot: RuntimeSnapshot = { tasks: [], connected: false };
  ...
  async start(workspaceRoot: string, vaultPassword?: string): Promise<void> {
    await this.stop(); this.root = await realpath(workspaceRoot); this.update({ connected: false, error: undefined, tasks: [] });
    ...
    const client = new RuntimeClient(runtimeClientOptions(connection)); this.client = client; await client.connect({ identity: { id: "vscode", name: "Visual Studio Code", adapterVersion: "0.2.1" }, capabilities }); this.unsubscribe = client.subscribeToEvents((value) => { this.event(value); void this.refresh(); }); await this.refresh();
  }
  async refresh(): Promise<void> { if (!this.client) return; try { const [status, tasks] = await Promise.all([this.client.getWorkspaceStatus(), this.client.listTasks()]); this.update({ status, tasks, connected: true, error: undefined }); } catch (error) { this.update({ connected: false, error: error instanceof Error ? error.message : String(error) }); } }
  async createTask(request: string, mode: TaskCreateInput["mode"], editorContext?: TaskCreateInput["editorContext"]): Promise<RuntimeTask> { if (!this.client) throw new Error("Runtime is not connected"); const id = `workspace:${createHash("sha256").update(this.root).digest("hex").slice(0, 20)}`; const task = await this.client.createTask({ request, mode, workspace: { workspacePath: this.root, repositoryRoot: this.root, workspaceId: id }, editorContext, ide: { id: "vscode", adapterVersion: "0.2.1", capabilities } }); await this.refresh(); return task; }
  ...
  async stop(): Promise<void> { this.unsubscribe?.(); this.unsubscribe = undefined; if (this.client) await this.client.disconnect().catch(() => undefined); this.client = undefined; if (this.process && !this.process.killed) this.process.kill(); this.process = undefined; this.snapshot = { tasks: [], connected: false }; }
}
