...
const fixture = async (name: string) => {
  ...
  const project = await discoverProject(root), memory = createFeltDBProjectMemory({ root, namespace: `pr9:${name}:${Math.random()}` }); await memory.initialize(project); await indexProjectIntoMemory(root, project, memory); return { root, project, memory };
};
...
describe("PR9 bounded autonomous execution certification", () => {
  ...
  it("keeps adaptive rerouting deterministic, permits runtime switches, and honors explicit override", async () => {
    const memory = createFeltDBProjectMemory({ root: "/tmp/pr9-routing", namespace: "pr9-routing" }); await memory.initialize({ id: "route", root: "/tmp/pr9-routing", name: "route", detectedLanguages: ["TypeScript"], packageManagers: [] });
