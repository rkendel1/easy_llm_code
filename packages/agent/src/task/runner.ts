...
export const createTaskRunner = (options: TaskRunnerOptions) => {
  ...
  const start = async (request: string, mode: TaskMode, identity?: { taskId: string; createdAt: string; checkpoint: TaskCheckpoint }, editorContext?: { activeFile?: string; selection?: unknown; cursor?: unknown; openFiles?: string[] }): Promise<TaskRunResult> => {
    ...
    try {
      ...
      const preferredFiles = [...new Set([editorContext?.activeFile, ...(editorContext?.openFiles ?? [])].filter((item): item is string => Boolean(item)))];
      const context = await contextEngine.build({ request, preferredFiles }); activeContext = context; await emit(taskId, { type: "context.completed", metrics: context.metrics });
      ...
      if (mode === "ask") {
        const editorPrompt = editorContext ? `\n\nEditor context: ${JSON.stringify(editorContext)}\nTreat activeFile as the current file. Base the answer on the supplied file contents; do not guess about files that are absent.` : "";
        const modelStartedAt = Date.now(), executed = await executeWithModelFallback(options.memory, activeDecision, (candidate) => options.askLlm ? options.askLlm({ request: `${request}${editorPrompt}`, context, model: candidate.model }) : invokeModel(`${request}${editorPrompt}\n\nRepository context: ${JSON.stringify(context)}`, { model: candidate.model })), answer = executed.value, metadata = answer && typeof answer === "object" ? answer as { model?: string; provider?: string; usage?: { inputTokens?: number; outputTokens?: number; estimatedCost?: number } } : {};
