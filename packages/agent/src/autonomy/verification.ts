...
import { runVerification, type VerificationCommandExecutor } from "../verification/runner.js";
...
export const runLayeredVerification = async (project: Project, taskId: string, proposalId: string, requested: VerificationStep[], policy: ExecutionPolicy, options: { risk?: RiskLevel; executor?: VerificationCommandExecutor } = {}): Promise<LayeredVerificationResult> => {
  ...
  for (const [index, scope] of scopes.entries()) {
    const run = await runVerification(project, taskId, proposalId, grouped.get(scope)!, policy, options.executor); run.verificationScope = scope; run.verificationReason = index ? `escalated from ${scopes[index - 1]}` : "smallest sufficient trusted verification layer"; runs.push(run);
  ...
  if (!runs.length) { const run = await runVerification(project, taskId, proposalId, requested, policy, options.executor); run.verificationScope = "targeted"; run.verificationReason = "proposal verification"; runs.push(run); }
